...
import { ALCHEMY_KEY_OP, ALCHEMY_KEY_POLYGON, ALCHEMY_KEY_ARB, INFURA_KEY_POLYGON } from "../src/data/env.jsx";

import { faucetContract, faucetABI,
    cpDepositContract, cpDepositABI, 
    cptAddress, cptABI, cpABIPolygon,  } from "./modules/contracts.js";			// this is to pull the chain-specific contracts and ABIs as needed.


import { cptPolyAddr, cptOpAddr, cpMintOp, cpContractPolygon, 
    cpMinterABI, cpMintArb, cptArbAddr, cpFaucetArb, cptBaseAddr, cpMintBase } from '../src/data/contracts.jsx';
...
//     // },
//   } 
import { base } from '../src/data/customChains.jsx';		// mainnet, arbitrum
...
const alPolWs = webSocket('wss://polygon-mainnet.g.alchemy.com/v2/' + alchemyAPI_P);
const alOpWs = webSocket('wss://opt-mainnet.g.alchemy.com/v2/' + alchemyAPI_OP);
const alArbWs = webSocket('wss://arb-mainnet.g.alchemy.com/v2/' + alchemyAPI_ARB);
...
export function checkName(chainId) {
    if(chainId === 10){
...
export function checkSymbol(chainId) {
...
export function checkCount(chainId) {
...
export function getURI(id, chainId) {
    if(chainId === 8453){
        return myBaseClient.readContract({
            address: cpMintBase,
            abi: cpMinterABI,
...
// OK Polygon, OK OPTIMISM, OK Arbitrum, BASE
export function getOwner(id, chainId) {
    if(chainId === 8453){
        return myBaseClient.readContract({
            address: cpMintBase,
            abi: cpMinterABI,
...
// OK Polygon, OK OPTIMISM, OK Arbitrum, OK Base
export function getBalance(address,chainId) {
    if(chainId === 8453){
        return myBaseClient.readContract({
            address: cpMintBase,
            abi: cpMinterABI,
...
// OK Polygon, OK OPTIMISM, OK Arbitrum, OK Base
export async function getTreasury(chainId){

    let defaultCurrency = "ETH";        // op, arb
    let safeAddress = "0xC3DDF7AFDE9Fc8Bec6af792C44dd801534aB1385";     // polygon
    ...
    if(chainId === 8453){
        ...
        console.log("Safe Balance ("+defaultCurrency+"): " + depBalEth + " " + defaultCurrency);
        ...
        console.log("Polygon Contract Balance: " + gameBalEth + " " + defaultCurrency);
        console.log("Safe Balance ("+defaultCurrency+"): " + depBalEth + " " + defaultCurrency);
